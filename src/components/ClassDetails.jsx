...
const ClassDetails = ({ classItem }) => {
    ...
    useEffect(() => {
        const fetchClassDetails = async () => {
            try {
                ...
                const attendance = data.sessions?.find(s => s.date === selectedDate)?.attendance || null;
    ...
    return (
        <div>
            <h2 className="text-xl font-bold mb-4">{classItem.name} - Attendance Details</h2>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            {dailyData ? (
                                <div className="space-y-4">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="p-4 bg-green-50 rounded-lg">
                                            ...
                                            <p className="text-2xl font-bold text-green-700">{dailyData.presentCount}</p>
                                        </div>
                                        <div className="p-4 bg-red-50 rounded-lg">
                                            <p className="text-sm text-red-600">Absent</p>
                                            <p className="text-2xl font-bold text-red-700">{dailyData.absentCount}</p>
                                        </div>
                                    </div>
                                ...
                                <p className="text-center text-muted-foreground">No attendance data for this date.</p>
                            )}
                        </CardContent>
                    </Card>
                </div>
